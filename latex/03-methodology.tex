...
\subsection{Classical Data Augmentation}
Classical data augmentation was applied to two of the training settings and was limited to annotation-preserving augmentations. These included intensity scaling and contrast adjustment to simulate variations in CT conditions. Geometric augmentations such as rotation, flipping, scaling, or cropping were avoided to prevent spatial misalignment between images and bounding box annotations. These augmentations preserve bounding box geometry and do not require changes to annotation coordinates.
...
Four training settings are evaluated:
\begin{itemize}
    \item Training on real images only (baseline).
    \item Training on real images with classical augmentation.
    \item Training on a combination of real and CycleGAN-generated images.
    \item Training on a combination of real images, classical augmentation, and CycleGAN-generated images.
\end{itemize}
All models are trained using identical optimization settings, including learning rates, batch sizes and number of epochs, to ensure a fair comparison.
